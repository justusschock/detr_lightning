...
class DetrSegm(Detr):
    def __init__(
        ...
        super().__init__(
            backbone, transformer, num_classes, num_queries, aux_outputs, ndim
        )
    ...
    def forward(
        ...
        seg_masks = self.mask_head(
            _src_proj,
            bbox_mask,
            [feature_tensors[2], feature_tensors[1], feature_tensors[0]],
        )

        outputs_seg_masks = seg_masks.view(
            feature_tensors[-1].size(0), self.num_queries, *seg_masks.shape[2:]
        )
        ...
        if self.aux_loss:
            return (
                ...
                [
                    ...
                    for aux_cls_output, aux_box_output in zip(
